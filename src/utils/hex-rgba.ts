...
type hexObject = { r: string; g: string; b: string; a: string };
type decimalObject = { r: number; g: number; b: number; a: number };
...
const formatRgb = (decimalObject: decimalObject, parameterA: any) => {
 ...
 */
const hexToRgba = (hex: string, a = "") => {
  const hashlessHex: string = removeHash(hex);
  const hexObject: hexObject = parseHex(hashlessHex);
  const decimalObject: decimalObject = hexesToDecimals(hexObject);
